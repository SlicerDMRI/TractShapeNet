    ...
    def convert_from_polydata(self, input_vtk_polydata, points_per_fiber=None):
            ...
            if array.GetName() == 'FA1':
                ...
                for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
                    ...
                    for pidx in range(0, ptids.GetNumberOfIds()):
                        FA1_list.append(array.GetTuple(ptids.GetId(pidx))[0])
            elif array.GetName() == 'FA2':
                input_vtk_polydata.GetLines().InitTraversal()
                for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
                    #print(lidx)
                    ptids = vtk.vtkIdList()
                    input_vtk_polydata.GetLines().GetNextCell(ptids)
                    for pidx in range(0, ptids.GetNumberOfIds()):
                        FA2_list.append(array.GetTuple(ptids.GetId(pidx))[0])
            elif array.GetName() == 'trace1':
                input_vtk_polydata.GetLines().InitTraversal()
                for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
                    #print(lidx)
                    ptids = vtk.vtkIdList()
                    input_vtk_polydata.GetLines().GetNextCell(ptids)
                    for pidx in range(0, ptids.GetNumberOfIds()):
                        trace1_list.append(array.GetTuple(ptids.GetId(pidx))[0])
                ...
                for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
                    ...
                    input_vtk_polydata.GetLines().GetNextCell(ptids)
                    for pidx in range(0, ptids.GetNumberOfIds()):
                        trace2_list.append(array.GetTuple(ptids.GetId(pidx))[0])
            elif array.GetName() == 'Nos':
                input_vtk_polydata.GetLines().InitTraversal()
                for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
                    #print(lidx)
                    ptids = vtk.vtkIdList()
                    input_vtk_polydata.GetLines().GetNextCell(ptids)
                    for pidx in range(0, ptids.GetNumberOfIds()):
                        Nos_list.append(array.GetTuple(ptids.GetId(pidx))[0])
        ...
        for lidx in range(0, input_vtk_polydata.GetNumberOfLines()):
            ...
            for pidx in range(0, ptids.GetNumberOfIds()):
                ...
                fiber_array_r.append(point[0])
                fiber_array_a.append(point[1])
                fiber_array_s.append(point[2])

        self.fiber_array_r = fiber_array_r
        self.fiber_array_a = fiber_array_a
        self.fiber_array_s = fiber_array_s
        self.FA1=FA1_list
        self.FA2=FA2_list
        self.trace1=trace1_list
        self.trace2=trace2_list
        self.Nos = Nos_list
